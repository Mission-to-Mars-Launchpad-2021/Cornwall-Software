 ...
 // extract from example, defines the down-facing sensor
  ...
  void setup() {
  Serial.begin(115200); // Open serial monitor at 115200 board to see ping results.
  ...
  //If the sensor detects something more than 15cm away, the rover creeps up to an edge stops, reverses and stops again
  do{
    slowdrive(0);
  ...
  while (sonar.ping_cm() <= 15);{
    ...
    //After stopping on the edge, the rover pauses, then reverses back to a safe distance
    Serial.println("Braking ");
    brake(5000);
  ...
  //Custom functions to control rover movements
